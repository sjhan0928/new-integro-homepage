...
export function CompanyCTASection({ }: CompanyCTASectionProps) {
  const { t } = useI18n();
  ...
  return (
    <section className="relative py-20 sm:py-28 md:py-36 lg:py-44 overflow-hidden bg-gray-100 dark:bg-gray-900">
      ...
      <div className="absolute inset-0 overflow-hidden">
        {/* 원형 데코레이션 */}
        <div className="absolute top-10 left-10 w-32 h-32 border border-primary/10 dark:border-primary/20 rounded-full" />
        <div className="absolute bottom-20 right-20 w-48 h-48 border border-[#3ecf8e]/10 dark:border-[#3ecf8e]/15 rounded-full" />
        <div className="absolute top-1/3 right-1/4 w-24 h-24 border-2 border-primary/5 dark:border-primary/10 rounded-full animate-pulse" />
        
        {/* 격자 패턴 */}
        <div className="absolute inset-0 bg-[linear-gradient(rgba(13,79,139,0.02)_1px,transparent_1px),linear-gradient(90deg,rgba(13,79,139,0.02)_1px,transparent_1px)] dark:bg-[linear-gradient(rgba(59,130,246,0.03)_1px,transparent_1px),linear-gradient(90deg,rgba(59,130,246,0.03)_1px,transparent_1px)] bg-[size:60px_60px]" />
        
        {/* 도트 패턴 */}
        <div className="absolute bottom-16 left-16 grid grid-cols-4 gap-3 opacity-15 dark:opacity-20">
          {Array.from({ length: 16 }).map((_, i) => (
            <div key={i} className="w-2 h-2 rounded-full bg-primary/50" />
          ))}
        </div>
        <div className="absolute top-20 right-16 grid grid-cols-3 gap-2 opacity-10 dark:opacity-15">
          {Array.from({ length: 9 }).map((_, i) => (
            <div key={i} className="w-1.5 h-1.5 rounded-full bg-[#3ecf8e]/60" />
          ))}
        </div>
      </div>
